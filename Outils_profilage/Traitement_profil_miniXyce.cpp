...
/*
  Ce code est lancé automatiquement par le script bash
  Il sert à calculer la moyenne, l'écart-type et le pourcentage de temps total
  à partir du fichier de données brutes obtenu avec le script bash
  Paramètres :
  - argv[1] : fichier de résultats
  - argv[2] : nombre de répétitions des mesures
  - argv[3] : nombre de tailles de communicateurs différents mesurées
*/
...
int main(int argc, char **argv)
{
  ...
  if(argc != 4)
  {
    std::cerr << "Mauvaise utilisation : " << argv[0] << " fichier nbrep nbproc" << std::endl;
    return 1;
  }

  const int NB_REP = atoi(argv[2]);
  const int NB_PROC = atoi(argv[3]);
