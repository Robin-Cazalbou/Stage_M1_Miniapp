...
std::vector<double> mX_linear_DAE_utils::evaluate_b(double t, mX_linear_DAE* dae)
{
  ...
  for (auto it2 = dae->b.begin(); it2 != dae->b.end(); it2++)
  {
    if (!(*it2))
    {
      result.push_back(0);
    ...
    else
    {
      ...
      for(auto it3 = (*it2)->scaled_src_list.begin(); it3 != (*it2)->scaled_src_list.end(); it3++)
      {
...
void mX_linear_DAE_utils::evaluate_b_omp(double t, mX_linear_DAE* dae, std::vector<double>& result)
{
  // given a linear DAE "A x + B x_dot = b(t)"
  // and a particular time point t
  // this function computes and returns the vector b at that time point t
  ...
  for(unsigned int i = 0; i < (dae->b).size(); i++)
  {
    ...
    else
    {
      ...
      for(auto it = (dae->b)[i]->scaled_src_list.begin(); it != (dae->b)[i]->scaled_src_list.end(); it++)
      {
	       mX_source* src = (*it)->src;
	       sum += src->output((double)(t)) * ((*it)->scale);
      }
