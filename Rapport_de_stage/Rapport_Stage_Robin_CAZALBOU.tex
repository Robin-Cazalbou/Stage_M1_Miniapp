...
% expliquer la méthode de mesure du temps de ces fonctions : conception de coroutines et d'une variable statique
% on appelle le timer en début et fin de fonction à mesurer, puis on appelle la coroutine en y mettant la valeur du temps écoulé. En fin de simulation, au moment d'afficher les valeurs de temps (et de les mettre dans un YAML), on appelle la coroutine avec un nombre négatif (par exemple -1) et cela retourne le temps total passé dans la fonction mesurée, cumulé sur tous les appels à cette fonction. Il faudra ensuite faire un traitement (en modifiant le code que j'avais écris) + faire un graphique comparatif + analyser et dire sur quelle fonction il faudra qu'on fasse des efforts
