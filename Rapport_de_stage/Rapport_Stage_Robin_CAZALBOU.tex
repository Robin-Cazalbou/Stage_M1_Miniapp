...
Tout d'abord, pour installer et compiler miniXyce à partir des codes sources, les étapes ont été très simples : le dépôt Github \cite{minixyce-code} propose une tarball contenant un Makefile ainsi que l'ensemble des fonctions de calcul, sans appel à une bibliothèque externe (autre que la bibliothèque standard). Un README d'une petite centaine de lignes présentes l'ensemble de la miniapp, de son installation et de son utilisation. Cela ne prend que quelques minutes.\bigskip
...
D'après les travaux de Aadithya, Keiter et Thornquist \cite{minixyce-validation}, les résultats obtenus par miniXyce ont pu être validés sur deux types de circuits différents. Nous allons essayer de répéter ces mesures afin de confirmer nous aussi cette conclusion.
Les deux exemples qui seront utilisés sont les suivants :
\begin{itemize}
\item une échelle RC, c'est-à-dire plusieurs condensateurs en parallèles séparés par des résistances,
\item un circuit RLC linéaire simple.
\end{itemize}\bigskip

MiniXyce et Xyce fonctionnent tous les deux en ligne de commande (une interface graphique est possible pour Xyce mais n'a pas d'intérêt dans notre étude et ne ferait que ralentir les temps d'exécution). De plus, ils ne nécessitent qu'une seul fichier en entrée, le reste des arguments constitue des options (sur les conditions initiales ou l'instant de début de simulation par exemple). Le fichier, nommé "netlist", contient toutes les informations sur les composants et l'architecture du circuit étudié. Le code source de miniXyce comporte de base des netlists de test issus des observations de \cite{minixyce-validation}.
...
% à quoi correspondent les résultats de Xyce et miniXyce ??

% expliquer le travail qui a été fait pour reconstruire les fichiers d'entrée de Xyce à partir des fichiers d'entrée de miniXyce



% utiliser les tests déjà présents dans miniXyce : comparer avec les résultats déjà proposés dans miniXyce (partie simple car miniXyce est surement codée pour dire si il y a concordance avec ces résultats) puis essayer de refaire la mesure dans Xyce pour voir si on a le même fichier de sortie (partie difficile : il faut comprendre quels tests ont été fait et comment utiliser Xyce pour les refaire)



\section{Profils de performance}

% partie performance : comparer les performances sur ces mêmes tests

% ne pas oublier de tester le séquentiel et le MPI 1 processsus (deux codes différents, pas compilés pareil)
