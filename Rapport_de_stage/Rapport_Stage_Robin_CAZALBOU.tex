...
La version de miniXyce 1.0.0 possède le désavantage d'avoir un Makefile supprimant les versions antérieures de l'exécutable, même si celui-ci est en séquentiel alors que l'on cherche à compiler en parallèle (et inversement). J'ai commencé par le modifier afin d'avoir deux versions qui puissent coexister, et que des dossiers soient générés pour séparer fichiers objets et exécutables. Il est ainsi possible de compiler successivement avec "make", puis avec "make -f Makefile.gnu.serial -B" pour obtenir les deux versions (l'option -B force la recompilation des fichiers objets qui seraient prévus pour une version parallèle).

Par ailleurs, le script de lancement des mesures a été étoffé de sorte à effectuer aussi le traitement en automatique, et à être plus générique, sans avoir à modifier le code source selon les choix de mesures (tailles de communicateurs MPI étudiées, nombre de répétitions).



\section{Nouveau profilage}

Les scripts Perl proposés dans le code source de miniXyce permettent de générer des échelles RC ou RLC aussi grandes que nécessaire. Deux scripts génèrent un même circuit à une différence près : dans le second, les composants électroniques sont groupés par types (résistances, condensateurs, bobines), alors qu'ils le sont par paquets R-C-L avec le premier. Cela peut avoir son importance selon le partage des tâches qui est fait entre processus. Nous ferrons donc nos mesures sur ces deux circuits : cirRLC100.net et cirRLC100\_bis.net, qui comportent 100 échelons chacun sur une échelle RLC.


% ajouter les figures + analyser : dire si les temps sont toujours les mêmes que précédemment avec le petit profil, etc.
...
\section{Profilage MAQAO}

Les circuits précédents s'exécutant en moins d'une seconde, l'outil de profilage MAQAO considère que les mesures sont trop courtes. Nous allons donc créer deux circuits plus imposants (10 fois plus gros) : cirRLC1000.net et cirRLC1000\_bis.net .

Après avoir effectué les mesures sur la version parallèle pour 1, 2, 3 et 4 processus MPI, le rapport MAQAO ainsi que le temps d'exécution du programme m'ont très vite indiqué que la version de miniXyce 1.0.0 était catastrophique en terme d'utilisation de MPI : avec seulement 3 processus, 97\% du temps d'exécution était passé dans des appels MPI (ce qui est beaucoup trop) pour un temps de calcul dépassant les 2 minutes.

Un rapide aperçu du rapport MAQAO avec le même nombre de processus, mais pour la version 1.1 présente seulement 10\% du temps en appel MPI, pour un temps d'exécution inférieur à 1 seconde (cf figure \ref{maqao_minixyce_1.0_vs_1.1}).

\begin{figure}
\begin{center}
\includegraphics[scale=0.45]{Images-Rapport/maqao_minixyce_1.0_3proc}
\includegraphics[scale=0.45]{Images-Rapport/maqao_minixyce_1.1_3proc}
\caption{Rapports MAQAO des versions 1.0 (à gauche) et 1.1 (à droite) de miniXyce effectués avec 3 processus MPI sur les circuits cirRLC1000.net et cirRLC1000\_1.1.net (1000 échelons RLC).}
\label{maqao_minixyce_1.0_vs_1.1}
\end{center}
\end{figure}
...
Au vu des performances sur le profil MAQAO, il me semble plus que judicieux d'étudier plus en détails cette version 1.1 .

\subsection{Différences entre versions 1.0 et 1.1}
