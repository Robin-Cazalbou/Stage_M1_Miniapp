...
Les scripts Perl proposés dans le code source de miniXyce permettent de générer des échelles RC ou RLC aussi grandes que nécessaire. Le temps actuel pour 4 résistances étant de l'ordre de la milliseconde, il me semble pertinent de générer un circuit qui prenne au moins 1 seconde à être analysé par la miniapp, c'est-à-dire au moins 100 plus grand.

Deux scripts génèrent un même circuit à une différence près : dans le second, les composants électroniques sont groupés par types (résistances, condensateurs, bobines), alors qu'ils le sont par paquets R-C-L avec le premier. Cela peut avoir son importance selon le partage des tâches qui est fait entre processus. Nous ferrons donc nos mesures sur ces deux circuits : cirRLC100.net et cirRLC100\_bis.net .
